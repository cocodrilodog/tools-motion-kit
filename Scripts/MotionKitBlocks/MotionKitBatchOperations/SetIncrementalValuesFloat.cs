namespace CocodriloDog.MotionKit {
	...
	/// <summary>
	/// Sets the values of the <see cref="MotionFloatBlock"/> at <see cref="MotionKitBatchOperationPath.Path"/>
	/// in a incremental way proportional to the <c>index</c> of the block.
	/// </summary>
	[Serializable]
	public class SetIncrementalValuesFloat : SetIncrementalValuesBase<float> {
		...
		protected override bool PerformOnChildBlock(ref MotionKitBlock childBlock, int index) {
			var motionFloatBlock = childBlock as MotionFloatBlock;
			if (motionFloatBlock != null) {
				motionFloatBlock.InitialValue = BaseInitialValue + index * InitialValueIncrement;
				motionFloatBlock.FinalValue = BaseFinalValue + index * FinalValueIncrement;
				return true;
			} else {
				return false;
			}
